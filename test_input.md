# ``cgb_input_generator`` Input File overview

``cgb_input_generator`` is specified in JSON format, which essentially defines elements as dictionaries and lists thereof. In the case of ``cgb``, the JSON file contains three primary entries (TF, motifs and genomes), and a flurry of configuration parameters. The following sections detail each of these.
...
#### This field, **Genomes**, is created and populated by ``cgb_input_generator``
...
## ``cgb_input_generator`` parameters
